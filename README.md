...
- ✅ Поддержка моделей OpenAI, Anthropic и Google (через официальное SDK)
- ✅ Выбор температуры генерации (0.0 - 1.0, шаг 0.1)
...
> ⚠️ **Важно:** Для работы Google моделей (Gemini, Veo) требуется Python 3.10+
...
**Google:**
- `gemini-2.5-flash` — быстрая текстовая модель
- `gemini-2.5-pro` — продвинутая текстовая модель
- `veo-3.1-generate-preview` — генерация видео (8 секунд, 720p/1080p)

> ⚠️ **Важно:** Генерация видео через Veo занимает от 11 секунд до 6 минут. Видео хранится на сервере 2 дня.
